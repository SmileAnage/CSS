...
>* **px** 像素单位
>* **%** 百分百单位(相对于父元素)
>* **em** 相对单位(相对于父元素) **1em=16px**通常用于移动端(常用1.5em)
>* **rpx** 小程序页面的单位
>当页面元素内容大小超出元素宽度
>>**overflow: hidden** 超出部分隐藏
>>**overflow: auto** 滚动条显示内容
### 2. 颜色
>* 元素的字体，背景，边框
>* 使用方式
>>1. 英文单词
>>2. 16进制(长的：#FFFFFF, #000000)
>>3. 16进制(短的：#fff, #000)
>* RGB(红色，绿色，蓝色) rgb(0-255, 0-255, 0-255) rgb(0,0,0)=#000
>* RGBA(红色，绿色，蓝色，透明度(0~1)) rgba(0,0,0,0.5)
>* **opacity: 0.7**设置透明度
## 七、盒子模型
### 1. 边框实现
语法：
